...
namespace TextractProcessor
{
    public class Function
    {
        ...
        public Function()
        {
            ...
            var modelConfig = BedrockModelConfig.TitanTextExpress; // or BedrockModelConfig.Claude3Haiku
        ...
        public async Task<ProcessingResult> FunctionHandler(ILambdaContext context)
        {
            try
            {
                ...
                // Process with schema mapper using simplified response
          ...
          string.Empty,
                ...
                context.Logger.LogInformation($"- Input tokens: {result.InputTokens}");
                context.Logger.LogInformation($"- Output tokens: {result.OutputTokens}");
                context.Logger.LogInformation($"- Estimated cost: ${result.TotalCost:F4}");
                context.Logger.LogInformation($"- Mapped file saved to: {result.MappedFilePath}");
