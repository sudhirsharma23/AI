...
//// Configure Redis distributed cache (from REDIS_CONNECTION environment variable)
//IDistributedCache? redisCache = null;
//var redisConn = Environment.GetEnvironmentVariable("REDIS_CONNECTION");
//if (!string.IsNullOrWhiteSpace(redisConn))
//{
//    var redisOptions = new RedisCacheOptions { Configuration = redisConn };
//    redisCache = new RedisCache(redisOptions);
//    Console.WriteLine("Redis cache configured from REDIS_CONNECTION.");
//}
//else
//{
//    Console.WriteLine("REDIS_CONNECTION not set; proceeding without Redis cache.");
//}

//await RunAsync(redisCache);

//// Asynchronous method to process all image URLs (local function)
//async Task RunAsync(IDistributedCache? redisCache)
//{
...
//                if (redisCache != null)
//                {
//                    var cachedBytes = await redisCache.GetAsync(cacheKey);
//                    if (cachedBytes != null && cachedBytes.Length >0)
//                    {
//                        var cachedJson = Encoding.UTF8.GetString(cachedBytes);
//                        Console.WriteLine("Cache hit (Redis) - using cached chat completion.");
...
//                if (redisCache != null)
//                {
//                    var bytes = Encoding.UTF8.GetBytes(completionJson);
//                    var distOptions = new DistributedCacheEntryOptions
//                    {
//                        AbsoluteExpirationRelativeToNow = TimeSpan.FromDays(7),
//                        SlidingExpiration = TimeSpan.FromHours(24)
//                    };
//                    await redisCache.SetAsync(cacheKey, bytes, distOptions);
//                }
